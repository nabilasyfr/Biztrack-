...
<div class="text-center mb-4 d-none d-print-block">
    ...
    <p>Per {{ \Carbon\Carbon::parse($dateTo)->format('d F Y') }}</p>
...
<div class="alert alert-warning mb-3" style="font-size:13px;border-radius:10px">
    ...
    Total Aset (Rp{{ number_format($totalAssets,0,',','.') }}) ≠
    Total Kewajiban+Modal (Rp{{ number_format($totalLiabilities+$totalEquity,0,',','.') }}).
    ...
    <div class="col-md-4">
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="stat-value" style="font-size:17px;color:#1a56db">Rp{{ number_format($totalAssets,0,',','.') }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="stat-value" style="font-size:17px;color:#dc2626">Rp{{ number_format($totalLiabilities,0,',','.') }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="stat-value" style="font-size:17px;color:#16a34a">Rp{{ number_format($totalEquity,0,',','.') }}</div>
        ...
        <div class="card h-100">
            ...
            <div class="card-body" style="padding:20px 24px">
                <div style="text-align:center;margin-bottom:16px">
                    ...
                    <div style="font-size:12px;color:#64748b">Per {{ \Carbon\Carbon::parse($dateTo)->format('d F Y') }}</div>
                </div>
                ...
                <div class="bs-row">
                    <span class="text-muted ps-3">
                        <code style="font-size:10px">{{ $a->code }}</code> {{ $a->name }}
                    </span>
                    <span class="fw-semibold">Rp{{ number_format($a->balance,0,',','.') }}</span>
                </div>
                @empty
                ...
                <div class="bs-total">
                    ...
                    <span class="text-primary">Rp{{ number_format($totalAssets,0,',','.') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-6">
        <div class="card h-100">
            ...
            <div class="card-body" style="padding:20px 24px">
                ...
                <div class="bs-row">
                    <span class="text-muted ps-3">
                        <code style="font-size:10px">{{ $l->code }}</code> {{ $l->name }}
                    </span>
                    <span class="fw-semibold">Rp{{ number_format($l->balance,0,',','.') }}</span>
                </div>
                @empty
                ...
                <div class="bs-total">
                    ...
                    <span class="text-danger">Rp{{ number_format($totalLiabilities,0,',','.') }}</span>
                </div>
                ...
                <div class="bs-row">
                    <span class="text-muted ps-3">
                        <code style="font-size:10px">{{ $e->code }}</code> {{ $e->name }}
                    </span>
                    <span class="fw-semibold">Rp{{ number_format($e->balance,0,',','.') }}</span>
                </div>
                @endforeach
                {{-- Laba/Rugi Berjalan --}}
                <div class="bs-row">
                    <span class="text-muted ps-3 fst-italic">
                        {{ $currentProfit >= 0 ? 'Laba' : 'Rugi' }} Periode Berjalan
                    </span>
                    <span class="fw-semibold {{ $currentProfit>=0?'text-success':'text-danger' }}">
                        {{ $currentProfit<0?'(':'' }}Rp{{ number_format(abs($currentProfit),0,',','.') }}{{ $currentProfit<0?')':'' }}
                    </span>
                </div>
                <div class="bs-total">
                    <span>Total Ekuitas</span>
                    <span class="text-success">Rp{{ number_format($totalEquity,0,',','.') }}</span>
                </div>
