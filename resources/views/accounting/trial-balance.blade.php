...
<div class="text-center mb-4 d-none d-print-block">
    ...
    <div style="font-size:13px;color:#475569;margin-top:4px">Periode: {{ \Carbon\Carbon::parse($month.'-01')->format('F Y') }}</div>
    <div style="font-size:11px;color:#94A3B8;margin-top:2px">Dicetak: {{ now()->format('d M Y H:i') }}</div>
    ...
    <div class="card-body py-3">
        <form method="GET" class="row g-2 align-items-end">
            <div class="col-auto">
                ...
                <input type="month" name="month" class="form-control form-control-sm" value="{{ $month }}" style="border-radius:8px">
            </div>
            <div class="col-auto">
...
{{-- ── Balance status ──────────────────────────────────────── --}}
<div class="balance-alert {{ $isAdjBalanced ? 'ok' : 'fail' }} no-print">
    <span class="alert-icon">{{ $isAdjBalanced ? '✅' : '⚠️' }}</span>
    <div>
        <strong>{{ $isAdjBalanced ? 'Neraca Seimbang' : 'Neraca Tidak Seimbang' }}</strong>
        &mdash; Periode {{ \Carbon\Carbon::parse($month.'-01')->format('F Y') }}
        @if(!$isAdjBalanced)
            &nbsp;· Selisih: <strong>Rp{{ number_format($adjDiff, 0, ',', '.') }}</strong>
        @endif
    </div>
</div>
...
<div class="kpi-grid">
    <div class="kpi-card" style="--kpi-accent:#2563EB;--kpi-icon-bg:#EFF6FF">
        ...
        <div class="kpi-value">Rp{{ number_format($totalDr,0,',','.') }}</div>
    ...
    <div class="kpi-card" style="--kpi-accent:#16A34A;--kpi-icon-bg:#F0FDF4">
        ...
        <div class="kpi-value">Rp{{ number_format($totalCr,0,',','.') }}</div>
        <span class="kpi-badge {{ $isBalanced ? 'ok' : 'fail' }}">
            <i class="bi bi-{{ $isBalanced ? 'check' : 'x' }}-circle-fill"></i>
            {{ $isBalanced ? 'Seimbang' : 'Selisih: Rp'.number_format($diff,0,',','.') }}
        </span>
    </div>
    <div class="kpi-card" style="--kpi-accent:#7C3AED;--kpi-icon-bg:#F5F3FF">
        ...
        <div class="kpi-value">Rp{{ number_format($totalAdjDr,0,',','.') }}</div>
    ...
    <div class="kpi-card" style="--kpi-accent:#0891B2;--kpi-icon-bg:#ECFEFF">
        ...
        <div class="kpi-value">Rp{{ number_format($totalAdjCr,0,',','.') }}</div>
        <span class="kpi-badge {{ $isAdjBalanced ? 'ok' : 'fail' }}">
            <i class="bi bi-{{ $isAdjBalanced ? 'check' : 'x' }}-circle-fill"></i>
            {{ $isAdjBalanced ? 'Seimbang' : 'Tidak Seimbang' }}
        </span>
    </div>
    <div class="kpi-card" style="--kpi-accent:{{ $isBalanced ? '#16A34A' : '#DC2626' }};--kpi-icon-bg:{{ $isBalanced ? '#F0FDF4' : '#FEF2F2' }}">
        <div class="kpi-icon"><i class="bi bi-{{ $isBalanced ? 'shield-check' : 'exclamation-diamond' }}"></i></div>
        <div class="kpi-label">Selisih (Sblm. Penyesuaian)</div>
        <div class="kpi-value" style="color:{{ $isBalanced ? '#16A34A' : '#DC2626' }}">
            {{ $isBalanced ? 'Rp0' : 'Rp'.number_format($diff,0,',','.') }}
        </div>
        <span class="kpi-badge {{ $isBalanced ? 'ok' : 'fail' }}">
            {{ $isBalanced ? 'Balance ✓' : 'Tidak Balance' }}
        </span>
    </div>
    <div class="kpi-card" style="--kpi-accent:{{ $adjEntries->count() > 0 ? '#F59E0B' : '#94A3B8' }};--kpi-icon-bg:{{ $adjEntries->count() > 0 ? '#FFFBEB' : '#F8FAFC' }}">
        <div class="kpi-icon"><i class="bi bi-wrench-adjustable"></i></div>
        <div class="kpi-label">Jurnal Penyesuaian</div>
        <div class="kpi-value">{{ $adjEntries->count() }}</div>
        <span class="kpi-badge {{ $adjEntries->count() > 0 ? 'info' : 'ok' }}">
            {{ $adjEntries->count() > 0 ? $adjEntries->count().' Entri' : 'Tidak Ada' }}
        </span>
    </div>
</div>
...
<div class="report-toolbar no-print">
    ...
    <div class="toolbar-right">
        <span style="font-size:12px;color:#94A3B8">{{ $balances->count() }} akun</span>
    </div>
</div>
        ...
        <table class="rpt-table" id="tblBefore">
            ...
            <tbody>
                ...
                @forelse($balances as $b)
                    @if(strtolower($b->type) !== $currentType)
                        @php $currentType = strtolower($b->type); @endphp
                        <tr class="group-sep">
                            <td colspan="5">
                                <span class="{{ $typePills[$currentType] ?? 'type-asset' }} type-badge">
                    ...
                    @endif
                    <tr>
                        <td class="td-code" style="width:80px">{{ $b->code }}</td>
                        <td class="td-name">{{ $b->name }}</td>
                        <td style="text-align:center">
                            <span class="{{ $typePills[strtolower($b->type)] ?? 'type-asset' }} type-badge">
                                {{ $b->type }}
                            </span>
                        </td>
                        <td class="num {{ $b->debit > 0 ? 'dr' : 'zero' }}">
                            {{ $b->debit > 0 ? 'Rp'.number_format($b->debit,0,',','.') : '—' }}
                        </td>
                        <td class="num {{ $b->credit > 0 ? 'cr' : 'zero' }}">
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="num {{ $isBalanced ? 'balanced' : 'unbalanced' }}">Rp{{ number_format($totalDr,0,',','.') }}</td>
                    <td class="num {{ $isBalanced ? 'balanced' : 'unbalanced' }}">Rp{{ number_format($totalCr,0,',','.') }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
...
<div class="report-toolbar no-print">
    ...
    <div class="toolbar-right">
        <span style="font-size:12px;color:#94A3B8">{{ $adjustedBalances->count() }} akun</span>
    </div>
</div>
    ...
    <div class="table-responsive">
        <table class="rpt-table" id="tblAfter">
            ...
            <tfoot>
                <tr>
                    <td colspan="3" style="padding-left:16px;font-weight:800;font-size:12.5px;letter-spacing:.04em;text-transform:uppercase">TOTAL</td>
                    <td class="num {{ $isAdjBalanced ? 'balanced' : 'unbalanced' }}">Rp{{ number_format($totalAdjDr,0,',','.') }}</td>
                    <td class="num {{ $isAdjBalanced ? 'balanced' : 'unbalanced' }}">Rp{{ number_format($totalAdjCr,0,',','.') }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
...
function exportExcel() {
    alert('Export Excel: Integrasikan dengan route /accounting/trial-balance/export?month={{ $month }}');
}
</script>
@endpush
