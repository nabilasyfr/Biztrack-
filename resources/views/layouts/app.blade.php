...
<head>
    ...
    <title>@yield('title', 'Dashboard') — BizTrack UMKM</title>
...
<nav id="sidebar">
    ...
    <div class="sidebar-footer">
        <div class="user-info">
            <div class="user-avatar">{{ strtoupper(substr(session('biztrack_name','?'), 0, 1)) }}</div>
            <div class="flex-fill overflow-hidden">
                <div class="user-name text-truncate">{{ session('biztrack_name') }}</div>
                <div class="user-role">{{ session('biztrack_role') === 'owner' ? 'Owner' : 'Kasir' }}</div>
            </div>
...
<div id="main-wrapper">
    ...
    <div id="topbar">
        ...
        <div class="topbar-right">
            ...
            @if($lowStockCount > 0)
            <a href="{{ route('products.index') }}?filter=low" class="btn btn-sm btn-outline-danger topbar-badge" title="{{ $lowStockCount }} produk stok rendah">
